  ...
  useEffect(() => {
    async function fetchPages() {
      try {
        ...
        if (response.data) {
          const pages = response.data.map((category) => ({
            title: category.name,
            href: `/category/${category.id}`,
          }))
          setAllPages(pages)
          setCurrentPageIndex(pages.findIndex((page) => page.href === pathname))
        }
      } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-5 border-t border-zinc-900/5 pt-8 sm:flex-row dark:border-white/5">
      ...
      <div className="flex gap-4">
        <SocialLink href="https://x.com/MetichaHQ" icon={XIcon}>
          Follow us on X
        </SocialLink>
        <SocialLink href="https://github.com/meticha" icon={GitHubIcon}>
          Follow us on GitHub
        </SocialLink>
        <SocialLink
          href="https://discord.com/channels/1281193953978482719/1281193956066988105"
          icon={DiscordIcon}
        >
